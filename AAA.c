...
// parametros: catalogo en memoria y archivo historial
...
void cargarCatalogoInicial(Nodo **cabeza, FILE **catalogo, char *nombreArchivo)
{
    ...
    *catalogo = fopen(nombreArchivo, "wb");
    ...
    // declaracamos una variable del tipo RegistroProducto para almacenar los datos de un producto
        ...
        fwrite(&r, sizeof(RegistroProducto), 1, *catalogo);
//cerramos el archivo catalogo para que los datos se guarden correctamente y el programa funcione mejor
    fclose(*catalogo);
...
void registroVentas ()
{
    ...
    // si el archivo no existe lo creamos si existe se sobrescribe
...
// el modoodo "r+b" abre el archivo binario para lectura y escritura, si el archivo no existe fopen retorna NULL
...
//leemos el valor de entregado y comprobamos que no falle (sin esto falla )
        ...
        if (!entregado) {
            //var con valor true para dsp pasarsela a la archvio
            bool nuevo = true;
...
//pos entregado apunta a el campo entregado asi que le sumamos el bool (entregado=fasle/true)
// y tmb el tamaño de cant items para poner el cursor donde arranca el 1er item
...
//si el pedido ya se entrego tenemos que mover el cursor al siguiente pedido
            int i;
            for (i = 0; i < cant_items; i++) {
                //MUY IMPORTANTE tenemos que calcular cuanto ocupa para saltarlo y poner el cursor al inicio del otro pedido
                //saltea la struct completa del producto, y el total
...
void mostrarColaPedidos()
{
    ...
    //contador para saber cuantos pedidos pendientes tenemos en total
    int totalPendientes = 0;
    //numerarador de pedidos
    int nro = 1;

    printf(" PEDIDOS pendientes \n");

    //se corta cuando no hay mas nada para leer
    while (1) {
        //nos sirven para guardar temporalmente lo que leemos del archvio
        int id_pedido;
        char cliente[40];
        ...
        //las variables que hicimos arriba
        //si alguna lectura nos falla es porque llego a su fin o hay error
        //y entonces cortamos todo y salimos del while
        if (fread(&id_pedido, sizeof(int), 1, f) != 1) {
            break;
        }
        if (fread(cliente, sizeof(cliente), 1, f) != 1) {
        ...
        //si no se entrego imprimimos la info
        if (!entregado) {
            printf("Pedido %d:\n", nro);
            printf("ID pedido: #%d\n", id_pedido);
            printf("Cliente: %s \n", cliente);
            ...
            int i;
            //si no se entrego recorremos todos los items
            for (i = 0; i < cant_items; i++) {
                //variables temporales para guardar lo que se lee del archivo
                RegistroProducto r;
                int cantidad;
                float subtotal;
        ...
        //si ya se entrego
            ...
            int i;
            for (i = 0; i < cant_items; i++) {
                //avanzamos el cursor, es = a la funcion anterior
                fseek(f, (int)sizeof(RegistroProducto) + sizeof(int) + sizeof(float), SEEK_CUR);
            }
        }
    }
...
void verHistorialPedidos (FILE*historial, char*nombre_historial)
{
    ...
    while (1) {
        ...
        //imrpimimos los datos
        printf("Pedido #%d | Cliente: %sTotal: $%.2f | Estado: ", id_pedido, cliente, total);

        //si se entrego lo imprimimos entregado y si no pendiente
        if (entregado) {
            printf("ENTREGADO");
        }
        else {
        ...
        //imprime cantidad de items de ese pedido
        printf(" Items: %d\n", cant_items);


        int i;
        //recorremos todos los items del pedido
        for (i = 0; i < cant_items; i++) {
            //como siempre lo imprimos
            RegistroProducto r;
            int cantidad;
            float subtotal;
...
void salir (Nodo*catalogo, Pedido*primero)
{
 ...
 //funcion para liberar el espacio usado por el catalogo para evitar memory leak
